...
if (isset($_POST['submit'])) {
    ...
    if (!$title) {
        ...
        if ($thumbnail['name']) {
            ...
            if (in_array($extension, $allowed_files)) {
                // make sure image is not too large. Max 2 Mb
                if ($thumbnail['size'] < 2000000) {
                    // upload thumbnail
                    move_uploaded_file($thumbnail_tmp_name, $thumbnail_destination_path);
                } else {
                    $_SESSION['edit-post'] = "Couldn't update post. Maximum size: 2Mb";
                }
            } else {
                $_SESSION['edit-post'] = "Couldn't update post. Extension must be png, jpg, or jpeg";
            }
        }
    }
    if ($_SESSION['edit-post']) {
        ...
        // sett thumbnail name if a new one ws uploaded, else keep old thumbnail name
