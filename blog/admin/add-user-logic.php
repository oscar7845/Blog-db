...
if(isset($_POST['submit'])) {
    ...
    if(!$firstname) {
        ...
        $_SESSION['add-user'] = "Minimum of 8 characters required";
    } elseif (!$avatar['name']) {
        ...
        if($createpassword !== $confirmpassword) {
            ...
            if(mysqli_num_rows($user_check_result) > 0) {
                ...
                if(in_array($extension, $allowed_files)) {
                    // make sure the file size is not too large (2mb+)
                    if($avatar['size'] < 2000000) {
                        // upload avatar
                        move_uploaded_file($avatar_tmp_name, $avatar_destination_path);
                    } else {
                        $_SESSION['add-user'] = "File size too big. Maximum size: 2Mb.";
                    }
                } else {
                    $_SESSION['add-user'] = "File can only be: png, jpg, or jpeg.";
                }
            }
        }
    }

    // redirect back to add-user page if there was eny problem
